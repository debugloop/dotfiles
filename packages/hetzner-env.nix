    ...
    modules = [
      {
        ...
        resource.hcloud_zone."danieln-de" = {
          name = "danieln.de";
          mode = "primary";
        };
        ...
        output.subaccounts = {
          value = lib.listToAttrs (map (hostName: {
              name = hostName;
              value = {
                host = "\${data.hcloud_storage_box.parent.server}";
...
in
  pkgs.writeShellScriptBin "hetzner-env" ''
    ${inputs.self.lib.mkHetznerEnv "env"}
    ...
    case "$CMD" in
      destroy)
        echo "Using destroy for the base env is a bad idea. The DNS zone holds manual records as well, etc." >&2
        exit 1
        ;;
      apply)
        echo "=== Running tofu apply on Hetzner environment ==="
...
EOF
      ...
      hostNames}
        ...
        ;;
      *)
        echo "Usage: hetzner-env [apply|destroy] [tofu-options...]" >&2
        exit 1
        ;;
    esac
  ''
