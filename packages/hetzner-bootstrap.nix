{
  ...
  pkgs.writeShellScriptBin "hetzner-bootstrap" ''
    ...
    echo "  2) Provision Hetzner env (keys + dns + storage box subaccounts) (hetzner-env)"
    echo "  3) Provision the host server (hetzner-server)"
    echo "  4) Install NixOS via nixos-anywhere (install)"
    echo
    echo "Each step is interactive and can be re-run independently."
    echo
    ...
    echo "  - Reset env (optional): nix run .#hetzner-env -- apply"
    echo
    echo "Note: install deletes the host private key after a successful run."
    ...
    prompt "Apply Hetzner env (keys + dns + storage box subaccounts)?"
    ...
    echo "3) Trigger a self-build on the host when convenient (optional)."
    echo
