    ...
    packages.storagebox-keygen = pkgs.writeShellScriptBin "storagebox-keygen" ''
      ...
      ${pkgs.lib.concatMapStrings (h: ''
            ...
            if [[ ! -f "keys/hosts/${h}.pub" ]]; then
              ...
              ${pkgs.coreutils}/bin/mkdir -p "modules/features/storage/storagebox"
              cat > "modules/features/storage/storagebox/_${h}.nix" <<EOF
          {
            host = "''${SB_HOST}";
            user = "''${SB_USER}";
          }
      ...
      ${pkgs.git}/bin/git add modules/features/storage/storagebox/_*.nix 2>/dev/null || true
    '';
  };
}
