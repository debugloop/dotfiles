{
  ...
  xdg.configFile."systemd/user/notify-failure@.service".text = ''
    [Unit]
  ...
  xdg.configFile."systemd/user/service.d/on-failure.conf".text = ''
    [Unit]
    OnFailure=notify-failure@%n
  '';

  xdg.configFile."systemd/user/notify-failure@.service.d/no-loop.conf".text = ''
    [Unit]
    OnFailure=
  '';
